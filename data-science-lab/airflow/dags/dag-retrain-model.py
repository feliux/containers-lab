...
def retrain_model():
    ...
    minio_client = Minio(endpoint="minio:9000", access_key=os.environ["MINIO_ACCESS_KEY"], secret_key=os.environ["MINIO_SECRET_KEY"], secure=False)
    logging.info("connected to minio")

    with NamedTemporaryFile() as tmp:
        minio_client.fget_object("sample-bucket", "sample-file.txt", tmp.name)
        #import the model
    ...
    with NamedTemporaryFile() as tmp:
        ...
        minio_client.fput_object(bucket_name="model", object_name="sample-data-{now_formated}.txt".format(**locals()), file_path=Path(tmp.name))
